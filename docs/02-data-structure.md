...
### 모호 질문 대응 가이드
- `휴가몇일`, `휴가 일수`, `휴가 며칠`처럼 붙여쓰기/축약어/구어체를 `synonyms`에 반드시 포함
- 한 키워드가 연차/경조휴가/병가 등 다중 의미를 가지면, 인덱스에서 각 faqId의 `weight` 차등 적용
- 상위 후보 점수 차이가 작으면(예: 1점 이내) 즉시 답변 대신 **명확화 질문**으로 전환
